...
## L-050 (02/09/2026) — Thêm field bắt buộc mới (target) làm vỡ fixture test cũ; và insert_line trùng dòng khai báo
- **Triệu chứng**: (a) Sau khi thêm `validateManualActivationTarget` + yêu cầu `manualException.target` khi enabled=true (GPT-REV-149), `test-gpt-approval-regress.mjs` FAIL `[FATAL] TỪ CHỐI (MANUAL_VALIDATION): MANUAL_TARGET_MISSING` vì fixture POLICY có `enabled:true` nhưng không có `target`; (b) khi chèn hàm mới vào `review-contract.mjs` bằng `editor insert_line`, `SyntaxError: Unexpected token 'export'` vì `new_text` đã kèm dòng `export function isManualApprovalValid(record, ctx) {` trong khi dòng đó đã tồn tại sẵn ở vị trí chèn → trùng khai báo.
- **Nguyên nhân gốc**: (a) thêm một ràng buộc mới vào validation path mà không rà toàn bộ fixture/phản ánh cảnh `enabled:true` trong các test suite (manual-exception + regress đều tự dựng POLICY literal); (b) `insert_line` chèn TRƯỚC dòng đích, nên nếu `new_text` kết thúc bằng chính dòng khai báo đã có ở dòng đích → 2 bản của cùng ký hiệu, body hàm lồng nhau → lỗi cú pháp tại dòng khai báo.
- **Tránh lặp lại**: (a) khi thêm field/ràng buộc bắt buộc vào validation (fail-closed), grep toàn bộ test fixture nào dựng policy/record chạm path đó và cập nhật target/field đúng — không chỉ suite mới viết; chạy toàn bộ suite ảnh hưởng (không chỉ suite vừa sửa) trước khi full-verify. (b) Khi `editor insert_line` muốn chèn hàm mới TRƯỚC một ký hiệu đã tồn tại, `new_text` KHÔNG được lặp lại dòng khai báo đích — chỉ chèn nội dung mới + đóng `}` và để dòng gốc ở lại; sau khi chèn chạy `node --check` file ngay để bắt duplicate/syntax sớm.
## L-051 (02/09/2026) — Truyền evidence artifact-shape vào bind check invocation-shape làm vỡ vì thiếu field
- **Triệu chứng**: `test-gpt-approval-manual-exception.mjs` FAIL `[FATAL] TỪ CHỐI (MANUAL_TARGET_WINDOW): MANUAL_TARGET_EVIDENCE_REPO_MISMATCH: structured evidence repository != invocation` khi early window check truyền `evidence: verifiedGptEvidence` (mock verifyGptEvidence) trực tiếp vào `validateManualActivationTarget`.
- **Nguyên nhân gốc**: `verifiedGptEvidence`/`record.gptEvidence` là artifact-shape (`authorLogin`/`issuer`/`policyDigest`/`decisionId`/`issuedAt`/`reviewDigest`...) — KHÔNG có field bind `repository`/`prNumber`/`headSha` ở top-level. `validateManualActivationTarget` có evidence-bind check so sánh `evidence.repository/prNumber/headSha/decisionId` với invocation → thiếu field → mismatch. Đây là 2 shape khác nhau (artifact vs invocation).
- **Tránh lặp lại**: khi truyền evidence vào một validator có bind-check invocation-shape, PHẢI chuẩn hóa evidence về `{ repository, prNumber, headSha, decisionId, issuedAt }` (khớp invocation) trước khi truyền — không truyền raw artifact object; nếu muốn thêm field mới (issuedAt) chỉ append vào object chuẩn hóa. Chạy `node --check` + test suite ngay sau khi sửa để bắt fail sớm (không đợi full-verify).
