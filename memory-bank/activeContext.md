...
## Vòng fix 4 findings PR #28 — GPT review (30/08/2026)
- **Trạng thái**: IN PROGRESS → đã fix xong, verify xong, chuẩn bị commit.
- **Bối cảnh**: GPT review PR #28 (Issue #25 Phase 4A) trả CHANGES_REQUESTED 4 blocking findings: breaker persist + atomic HALF_OPEN claim, auto-commit gate không suy diễn, DoD emit chỉ khi evidence canonical, git/gh lock cwd/repo/branch/HEAD + negative test.
- **Đã fix**:
  - `scripts/breaker-persist.mjs` (mới): atomic write/read (`atomicWriteJson`/`readJsonSafe`), file lock exclusive (`acquireLock`/`withFileLock`), load/save/namespace (`loadBreaker`/`saveBreaker`/`buildBreakerNamespace` + `safeFilePart` Windows-safe), `claimHalfOpenProbe` persist (cooldown, fail reset, success CLOSED).
  - `scripts/circuit-breaker.mjs`: tích hợp persist cho `claimHalfOpenProbe` (đọc/ghi file breaker per-tool namespace).
  - `scripts/execution-broker.mjs`: `shouldEmitDodEvent` gate `allPass === true` (fail-closed undefined/null); `toolVerifyStatus` trả `allPass: r.ok`; auto-commit gate dùng dữ liệu thật không suy diễn; git/gh lock cwd/repo/branch/HEAD.
  - Tests: `scripts/test-breaker-persist.mjs` (mới, 11 tests — gồm cross-process claim 5 child → 1 winner, git lock negative), cập nhật `test-circuit-breaker.mjs` (22), `test-execution-broker.mjs` (28), `package.json` (thêm suite vào test + MANDATORY_TEST_SUITES 5 suites).
- **Verify**:
  - `pnpm test` 308/308 PASS (pure-logic 229 + dod 18 + breaker 22 + broker 28 + breaker-persist 11) — đã thêm invariant `passed !== cases.length → exit 1` trong `test-execution-broker.mjs` chống tái phạm duplicate runner loop (root cause cũ `40/28`).
  - `node scripts/full-verify.mjs` 152/153 (1 FAIL pre-existing: `mcp-test-evidence/test-server.mjs` `test_failure_detail` — ngoài scope, không do thay đổi PR).
  - `node scripts/test-evidence-e2e.mjs` 23/23 PASS (sau khi restore `.agent/test-manifest.json` bị corrupt thành `{ invalid json` bởi test 60 block EVID_DIR — state bẩn, không phải regression).
- **Chẩn đoán "pnpm verify treo"**: KHÔNG treo thật — log `pnpm-verify.log` đã ghi đủ bảng `154/154 PASS`; chỉ là harness timeout khi redirect qua PowerShell `*>` trên output dài. Đã verify từng thành phần độc lập.
- **Next**: commit + push lên branch PR #28 → handoff review.

## Issue #25 Phase 4A — Deterministic DoD + Execution Broker + Circuit Breaker (30/08/2026)
- **Status**: COMPLETED — PR #28 mở, CI xanh.
- **Files**: `scripts/dod.mjs` (9-state DoD machine), `scripts/circuit-breaker.mjs` (per-tool breaker), `scripts/execution-broker.mjs` (CLI facade 6 tool + auto-commit gate 8 điều kiện), `scripts/test-dod.mjs`, `scripts/test-circuit-breaker.mjs`, `scripts/test-execution-broker.mjs`, `package.json`.
- **Verify**: `pnpm test` 276/276 (pure-logic 229 + dod 18 + breaker 16 + broker 13), `pnpm verify` RESULT PASS, CI Verify success @ HEAD `4d81a334500e1b4a4cfa1f759291eeda31843b0b`.
- **PR**: https://github.com/duongpdddic-droid/AI_PR_REVIEWER/pull/28 — churn 1291 (add 1290 + del 1) < policy maxLines 1500, base main, head `feat/issue-25-phase4a-deterministic-dod-clean`.
- **Branch đổi theo chỉ thị Bố**: stash Phase 4A riêng, checkout main@0cf7b18, branch mới `feat/issue-25-phase4a-deterministic-dod-clean` (KHÔNG chứa d7b92a8/Phase B), apply stash, drop. Branch cũ `feat/issue-25-phase4a-deterministic-dod` giữ nguyên, không push thêm, không force.
- **Audit policy (theo lệnh Bố)**: `diffLimits.maxLines = 1500` canonical (KHÔNG phải 100). Số 100 là default sai tự đặt trong `toolPreReviewStatus` → đã sửa → 1500. `evaluateDiffLimits` dùng `policy.diffLimits.maxLines`. Diff Phase 4A 1291 < 1500 → hợp lệ, không cần chia PR/override.
- **Learnings mới**: L-048 (isMain basename equality), L-049 (PowerShell stash@{0} single-quote), L-050 (không tự đặt default policy, đọc canonical).
- **Stash giữ nguyên**: `stash@{0}` memory-bank pre-PhaseB (AC14), `stash@{1}` wip/ac14-cline-mem-bridge-removeProject — không apply/drop.
- **Gateway (30/08 01:57)**: supervisor `--status` exit 0 nhưng `alive:false, ready:false, lease:null`; instance 15616 không còn process; health/ready/offset.json LastWriteTime 28/08 21:45 (stale). Đã ghi evidence + notify blocked (inbound có thể ngừng hoạt động — supervisor sẽ tự heal khi cần).
- **Bước tiếp theo (ngoài scope Phase 4A)**: GPT review PR #28; Phase 4B large-file context; tích hợp broker vào autonomous-run/unified-orchestrator.
