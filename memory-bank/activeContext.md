# Active Context
## Issue #38 — Round 6 (02/09/2026 01:52): GPT-REV-152 fix, HEAD `39d6258` — HANDOFF ACCEPTED → status:review-requested (MERGED → main `239380a`)
- **MERGED (02/09/2026 01:19 UTC)**: Bố xác nhận GPT approved tại exact HEAD `39d6258…`. Gate đọc-lại PASS đủ: PR #39 OPEN/MERGEABLE/non-draft, HEAD exact, comment handoff `5498797036` tồn tại, Verify CI COMPLETED/SUCCESS, không commit mới sau approval. `gh pr merge --squash --delete-branch=false` → merge commit `239380a544e34fcfd11e9b4d886485a4bd5d783e` = origin/main `239380a`. CI sau merge (commit `239380a`) COMPLETED/success (19:20:35Z). PR #39 state MERGED. Issue #38 giữ OPEN (`Ref #38`, không auto-close). Remote branch `feat/issue-38-reviewer-principal` đã xóa. Không provision credential; không bật manualException; `enabled:false` + `target:null` giữ nguyên; không re-run pre-review; không chạm PR #33/Issue #32; không deploy/self-approve.

## Issue #38 — Round 6 (02/09/2026 01:52): GPT-REV-152 fix, HEAD `39d6258` — HANDOFF ACCEPTED → status:review-requested
- **Nhiệm vụ**: xử lý GPT-REV-152 trên Issue #38 / PR #39 — manual approval activation time-window (không mở rộng scope).
- **Thực hiện**:
  1. [x] `review-contract.mjs` `validateManualActivationTarget`: thêm `activatedAt` + `expiresAt` vào required; validate ISO (regex + Date.parse), `expiresAt > activatedAt`, `duration <= activationTtlSeconds`, `now` ∈ [act,exp] (MANUAL_TARGET_NOT_ACTIVATED / EXPIRED), `evidence.issuedAt` ∈ [act,exp] (EVIDENCE_BEFORE_ACTIVATION / AFTER_EXPIRY — evidence mới không thể gia hạn activation).
  2. [x] `isManualApprovalValid` truyền `evidence` (chuẩn hóa từ record.gptEvidence), `nowMs` (ctx.nowMs), `ttlSeconds` (policy.activationTtlSeconds) vào target check.
  3. [x] `gpt-approval.mjs` `performManualApproval`: thêm early block validateManualActivationTarget TRƯỚC section 9 (ghi audit PASS) → throw `TỪ CHỐI (MANUAL_TARGET_WINDOW)` fail-closed trước mọi mutation/marker/audit SUCCESS.
  4. [x] Bump `policyVersion` `2026-09-02.0`→`2026-09-02.1`; sync `.github/ai-review-policy.json`, `project-registry.mjs CANONICAL_POLICY_VERSION`, `.agent/project.json`, fixtures project-registry (4), test-pure-logic (2×PV), test-gpt-approval-evidence/getgatestate/regress/manual-exception. Policy note cập nhật target window. Không đổi maxLines/allowedReason.
  5. [x] Negative tests: evidence +27 window assertions (ok, thiếu/malformed, expires<=act, TTL vượt, trước/sau activation, evidence trước/sau); manual-exception +5 integration (thiếu expiresAt/activatedAt, not-activated-yet, expired-window, ttl-exceeded).
- **Verify**: `node scripts/full-verify.mjs` **161/161 PASS exit 0**; evidence 80/80; manual-exception 51/51; regress 25/25; getgatestate 12/12; pure-logic 229/229; `node --check` 8 file OK; `git diff --check` sạch.
- **Commit**: `39d625804044c5bf4b47f4f4ba810d073a996d2e` push origin `feat/issue-38-reviewer-principal` (`5d7c625..39d6258`). Local = remote = PR HEAD.
- **HANDOFF ACCEPTED (round 3)**: `task_handoff` (#38, PR #39) — reportCommentId `5498797036`, digest `ff6b8b0bcf40787bf84d00fc8f3719b517ccbae8dfef0520a0f5d5cf37751c3b`, head exact PR HEAD `39d6258…`, labels `agent:gpt` + `status:review-requested`. Trước đó gỡ `review-requested`→`changes-requested` (task_review request-changes) rồi handoff. Issue #38 OPEN.
- **Deferred/Not touched**: reviewer-principal credential chưa provision; `manualException.enabled` giữ false + `target:null`; không chạm PR #33/Issue #32; Issue #41 (full-verify CWD) để agent:cline.


## Issue #38 — Round 5 (02/09/2026): GPT-REV-149..151 fix, HEAD `5d7c625` — HANDOFF ACCEPTED → status:review-requested
- **Nhiệm vụ**: xử lý GPT-REV-149/150/151 trên Issue #38 / PR #39 (không mở rộng scope).
- **Thực hiện**:
  1. [x] **GPT-REV-149 (activation target)**: `review-contract.mjs` thêm `validateManualActivationTarget` (bind exact repository + prNumber + full 40-hex headSha + decisionId; khớp invocation ↔ structured evidence ↔ policy activation target; target null/missing khi enabled → fail-closed). `isManualApprovalValid` chèn target check. `gpt-approval.mjs` audit entry thêm `target {repository,prNumber,headSha,decisionId}` binding. Policy thêm `manualException.target: null` (mặc định, khóa `enabled:false` + `target:null`). Negative tests: evidence + manual-exception (target-null/missing-field/repo/pr/head/decision mismatch).
  2. [x] **GPT-REV-150 (bump policyVersion)**: `2026-08-23.7` → `2026-09-02.0`; sync toàn bộ canonical pin/reference/test/digest: `project-registry.mjs` `CANONICAL_POLICY_VERSION`, `.agent/project.json`, fixtures project-registry (ai-pr-reviewer/duplicate-id/generic/qlda-dtxd), `test-pure-logic.mjs` (2×PV), `test-gpt-approval-evidence.mjs`, `test-gpt-approval-getgatestate.mjs`, `test-gpt-approval-regress.mjs`, `test-gpt-approval-manual-exception.mjs` (2×). KHÔNG đổi `diffLimits.maxLines` (1500) / `allowedReason` (chỉ PRE_REVIEW_DIFF_LIMIT).
  3. [x] **GPT-REV-151 (PR metadata)**: PR #39 body `Closes #38` → `Ref #38` (Issue #38 giữ OPEN); thêm section "Còn lại / Deployment gate" (provision reviewer-principal → enable + set target → real pilot).
  4. [x] **Follow-up issue #41** tạo cho `full-verify.mjs` mắc `process.cwd()` làm ROOT (không sửa trong PR #39) — agent:cline, status:ready-for-cline.
- **Verify**: `node scripts/full-verify.mjs` **161/161 PASS exit 0** (chạy từ đúng worktree root); `test-gpt-approval-evidence.mjs` 56/56; `test-gpt-approval-manual-exception.mjs` 46/46; `test-gpt-approval-regress.mjs` 25/25; `test-gpt-approval-getgatestate.mjs` 12/12; `test-pure-logic.mjs` 229/229; `test-project-registry.mjs` 136/136; `git diff --check` sạch.
- **Commit**: `5d7c625bf380c3130adc3c8ed0d4b3814c1281b2` push origin `feat/issue-38-reviewer-principal` (`4c03cdb..5d7c625`). Local = remote = PR HEAD.
- **HANDOFF ACCEPTED (lần 2)**: `mcp-task-server__task_handoff` (#38, PR #39) — handoffReport READY_FOR_REVIEW (local validate bằng contract module → OK=true), server accepted: reportCommentId `5498447694`, digest `889c292482489df2b29756736d88c2c465ba384ef2250d69a577eecbc911e44d`, Issue #38 → labels `agent:gpt` + `status:review-requested`, head = exact PR HEAD `5d7c625…`. Trước đó phải gỡ `status:review-requested` → `status:changes-requested` (vì GPT đã request changes 149..151) rồi mới handoff thành công. Issue #38 OPEN (Ref #38, không auto-close). KHÔNG merge/deploy/approve/enable policy.
- **Deferred**: reviewer-principal credential CHƯA provision (`reviewerAuthorityAllowlist` rỗng — fail-closed by design); `manualException.enabled` giữ false + `target:null`. Không chạm PR #33/Issue #32.
...
- **HANDOFF ACCEPTED (02/09/2026 00:38)**: `mcp-task-server__task_handoff` (#38, PR #39, repo `duongpdddic-droid/AI_PR_REVIEWER`) → hình. Contract v1.0.0 xác định từ module THẬT của running server `C:\Users\Admin\.cline\AI_PR_REVIEWER\scripts\review-handoff-contract.mjs` (peer) — schema: `delivery` phải FLAT {commitSha, pushResult, prActions, headReadBack, noApprovalClaim}; `findingResolution.items[*]` status ∈ {fixed,disputed,unresolved}; `tests.items[*]` đủ name/location/setup/assertions/negativeAssertion/realFs/result/exitCode; `codeEvidence.items[*]` đủ 10 field. Local validate bằng chính contract module (registeredRepos chứa repo) → ok=true status=READY_FOR_REVIEW, verifyHandoffIdentity ok, secret/abs=0 (digest `1fc6a4ac...`). Server accepted: reportCommentId `5497909019` (digest khớp), Issue #38 → labels `agent:gpt` + `status:review-requested`, head = exact PR HEAD `4c03cdb135c28b028457ea0ce23b46279128e6dd`. CI `Verify CI` success tại exact HEAD. KHÔNG bypass label trực tiếp; không mutate label/GPT approve/merge/deploy/enable policy.
